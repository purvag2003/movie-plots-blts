...
class MovieEdit extends React.Component {
    ...
    constructor(props) {
        ...
        this.state = {
            id: data.id, release_year: data.release_year, title: data.title, origin: data.origin, director: data.director, cast: data.cast, genre: data.genre, wiki: data.wiki, plot: data.plot,
        }
    }
    ...
    handleSave(event) {
        const data = {
            release_year: this.state.release_year, title: this.state.title, origin: this.state.origin, director: this.state.director, cast: this.state.cast, genre: this.state.genre, wiki: this.state.wiki, plot: this.state.plot,
        }
    ...
    render() {
        ...
        return <div className={"edit-box"}>
            ...
            <Row className={"edit-row"}>
                ...
                <Col md={3}>
                    <Form.Control placeholder="Genre" name={"genre"} value={this.state.genre} onChange={this.handleChange.bind(this)} />
            ...
            <Row className={"edit-row"}>
                <Col md={4}><Button onClick={this.handleSave.bind(this)}> Save </Button></Col>
            </Row>
        </div>
    }
}
